...
export async function POST(req: Request) {
  ...
  // Tentar com webhook secret, se falhar, processar sem verificação
  ...
  try {
    ...
    if (webhookSecret) {
      ...
      console.log('✅ WEBHOOK VERIFICADO COM SECRET')
    } else {
      console.log('⚠️ WEBHOOK SECRET NÃO ENCONTRADO - PROCESSANDO SEM VERIFICAÇÃO')
      event = JSON.parse(body) as Stripe.Event
    }
  } catch (err: unknown) {
    console.log('⚠️ ERRO NA VERIFICAÇÃO - TENTANDO PROCESSAR DIRETAMENTE')
    try {
      event = JSON.parse(body) as Stripe.Event
      console.log('✅ EVENTO PROCESSADO DIRETAMENTE')
    } catch (parseErr) {
      console.log('❌ ERRO AO PROCESSAR BODY:', parseErr)
  ...
  if (event.type.includes('subscription')) {
    ...
    // FORÇAR datas se não existirem
    let periodStart = (subscription as any).current_period_start
    let periodEnd = (subscription as any).current_period_end

    // Se não tem datas mas está ativa, criar datas padrão
    if (finalStatus === 'active' && !periodStart && !periodEnd) {
      console.log('=== CRIANDO DATAS PADRÃO PARA ASSINATURA ATIVA ===')
      const now = Math.floor(Date.now() / 1000)
      periodStart = now
      periodEnd = now + 365 * 24 * 60 * 60 // 1 ano
      console.log('Datas criadas - Start:', periodStart, 'End:', periodEnd)
    }
