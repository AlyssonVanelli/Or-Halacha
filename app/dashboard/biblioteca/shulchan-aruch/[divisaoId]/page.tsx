  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        if (divisionError) {
          errorHandler.handleError('Carregamento da divisão', divisionError, { divisaoId, userId: user.id })
          return
        }
        ...
        try {
          ...
          errorHandler.handleError('Verificação de acesso', accessError, { divisaoId, userId: user.id })
          ...
          const hasActiveSub = !!subscriptionData && new Date(subscriptionData.current_period_end) > new Date()
    ...
    loadData()
  }, [user, divisaoId, errorHandler.retryCount])
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 place-items-center">
                  ...
                  return (
                    <Link
                      ...
                      className="group relative overflow-hidden rounded-2xl bg-white shadow-lg transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 w-full max-w-xs"
                    >
                      {/* Header com gradiente */}
                      <div className={`h-20 bg-gradient-to-r ${colors[index % colors.length]} p-4`}>
