  ...
  useEffect(() => {
    ...
    async function checkPurchaseStatus() {
      ...
      try {
        ...
        // Apenas os tratados específicos comprados ficam desbloqueados
        const purchasedDivisionIds = ativos.map(pb => pb.division_id)

        setPurchasedBooks(purchasedDivisionIds)
      } catch (error) {
        console.error('❌ LIVRO PAGE - Erro na verificação:', error)
        setPurchasedBooks([])
      }
    }
    checkPurchaseStatus()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user?.id, livroId, book?.divisions])
